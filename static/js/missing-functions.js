...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.plsMissingFunctionsLoaded) {
        return;
    }
    window.plsMissingFunctionsLoaded = true;

    // Add any missing utility functions here
    ...
    // Socket.IO connection with proper error handling for disabled server
    let socket = null;
    ...
    function initializeSocket() {
        ...
        try {
            ...
            if (typeof io === 'undefined') {
                console.log('Socket.IO client not loaded - real-time features disabled');
                return;
            }
            ...
            socket.on('connect_error', function(error) {
                console.log('Socket connection error (expected if WebSocket disabled):', error.message);
                // Don't retry if server doesn't support WebSocket
                if (error.message.includes('4500') || error.message.includes('Internal server error')) {
                    socket.disconnect();
                    console.log('WebSocket disabled on server - switching to polling mode');
                }
            });
    ...
    // Initialize socket when page loads (only once)
    if (document.readyState === 'loading') {
